...
def make_performance_list(accuracy,precision,recall,f1):
    accuracy_list = []
    precision_list = []
    recall_list = []
    f1_list = []
...
'''
1. 역직렬화
2. 리스트 돌면서 VT 가져와서 list append
3. mal_vehicle이랑 비교하여 성능평가
4. 차트
'''
find_temp = []
for i in new_vlist:
...
find_temp_cut = find_temp_sort[:10]
find = []
...
make_performance_list(accuracy,precision,recall,f1)
