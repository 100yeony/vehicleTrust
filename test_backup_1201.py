...
for i in v_list_ :
    ...
    for key,info in i.items():
        for second_key in info:
            if int(info['vid']) in mal_vehicle:
                mal_user.append((info['vid'],info['user']))

mal_user = set(mal_user)
...
for i in reporters:
    ...
    for c in original_data:
        if i == c[10]:
            if c[15] != '':
                data = {'contents':c[14], 'score':0,'createdTime':(c[2],c[3]),'GPS':c[15], 'vid':f'{reporters.index(i)}', 'user':i}

        elif i in ['김영귀', '김성열']:
            if c[15] != '':
                maldata = {'contents':c[14], 'score':0,'createdTime':(c[2],c[3]),'GPS':c[15], 'vid':f'{reporters.index(i)}', 'user':i}
...
def mix_data(list):
    mal_list = []
    mal_v1_check = list.copy()
    mal_v2_check = list.copy()
    while len(mal_v1_check)!=0:
        mal_v1 = random.choice(mal_v1_check)
        mal_v2 = random.choice(mal_v1_check)
        if mal_v1 != mal_v2:
            mal_v1_data = ''
            mal_v2_data = ''

            #==============데이터 저장 ================
            for key, info in mal_v1.items():
                for second_key in info:
                    real_info = info['VDI']
                    for third_key in real_info:
                        mal_v1_data = real_info['createdData']

            for key, info in mal_v2.items():
                for second_key in info:
                    real_info = info['VDI']
                    for third_key in real_info:
                        mal_v2_data = real_info['createdData']
            ...
            for key, info in mal_v1.items():
                for second_key in info:
                    real_info = info['VDI']
                    for third_key in real_info:
                        real_info['createdData'] = mal_v2_data.copy()
                mal_list.append(info)

            mal_v1_check.remove(mal_v1)
            # mal_v2_check.remove(mal_v2)
            pprint(mal_list)
    return mal_list

# mal_list_= mix_data(new_list)
# print('############## mal_list_:', mal_list_, len(mal_list_))

#
# v1,v2 = mix_data_exchange(v1,v2)
# print('v1:', v1)
# print('------')
# print('v2:', v2)
