...
def txt2list() :
    lists = []
    file = open('./event_dataset/event_08_tab.txt', 'r')
    while True :
...
def make_mixed_data(reporters, original_data, mal_user):
    ...
    ################# mix data 직렬화 #######################
        with open('./_mixed_data/'+f'vehicle{i}_info.json', mode='w', encoding='UTF8') as f:
...
def carTrust(mal_vehicle):
    ...
    while len(v1_list_check)!=0:
        ...
        if v1_temp != v2_temp:
            # << 역직렬화 >>
            with open('./_mixed_data/'+f'{v1_temp}', 'rb') as f:
                v1 = json.load(f)
            with open('./_mixed_data/'+f'{v2_temp}', 'rb') as f:
                v2 = json.load(f)
    ...
    for i in final_vlist:
        ...
        for key, info in i.items():
            for second_key in info:
                ...
                # print(name)
        # << 직렬화 >>
        with open('./_data/'+f'vehicle{name}_info.json', mode='w', encoding='UTF8') as f:
            json.dump(i, f, cls=CustomEncoder, indent=2)
            # print('직렬화 complete!!')
    # print('new 직렬화 complete!')
        # << 역직렬화 >>
        # with open('./_data/'+f'vehicle{i}_info.json', 'rb') as f:
        #     data = json.load(f)
        #     print('역직렬화 complete!')
...
# # <<  original data 직렬화 >>
    with open('./_original_data/'+f'vehicle{i}_info.json', mode='w', encoding='UTF8') as f:
        json.dump(vehicle, f, cls=CustomEncoder, indent=2)
        # print('complete!!')
'''
# # << original data 역직렬화 >>
for i in reporters:
    with open('./_original_data/'+f'vehicle{i}_info.json', 'rb') as f:
        data = json.load(f)
