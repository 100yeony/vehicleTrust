...
def make_user_set(vInfo):
    users = []
    return users


# def make_mal_data(real_data, users, vInfo):
#     mal_data = real_data의 데이터를 mix
#
#     for i in real_data:
#
#
#
#     vInfo['contents'] = mal_data
#     return vInfo
...
v1_ = original_data[1:1288]
v2_ = original_data[1289:2478]
v3_ = original_data[2479:4275]
v4_ = original_data[4276:5595]
v5_ = original_data[5596:6292]
v6_ = original_data[6293:8000]

reporters = []
for i in original_data :

    users.append(i[13]) #접수자
    reporters.append(i[10])
    #vInfo = make_real_vehicleInfo(original_data, 0)
    # print(vInfo)
...
'''
vehicle = {'vid': 0, 'user':'', 'social_activities':{}, 'VDI':{}, 'VTI':{}, 'RTI':[]}
for i in users:
    idx = users.index(i)
    vehicle['vid'] = idx+1
    vehicle['user'] = i
    vehicle['social_activities'] = {'info':{},'scoring':0, 'ignoring':0, 'sharing':0}
    vehicle['VDI'] = [{'createdData':{'contents':[], 'score':0,'createdTime':0},'receivedData':{'contents':[], 'score':0,'createdTime':0}}]
    vehicle['VTI'] = {'DR':0,'AS':0,'NT':0,'UR':0, 'VT':0, 'connectivity':0, 'CF':0,'VT_updateTime':0, 'PVT':0}
    pprint(vehicle)

    changed_vInfo = make_real_vehicleInfo(original_data, i)
    print('changed_vInfo :', changed_vInfo)
    print('--------------------------------')
    ...
    with open('./data/'+f'vehicle{i}_info.json', 'rb') as f:
        if users.index(i) == 0:
            v1 = json.load(f)
        elif users.index(i) == 1:
            v2 = json.load(f)
        elif users.index(i) == 2:
            v3 = json.load(f)
        elif users.index(i) == 3:
            v4 = json.load(f)
        elif users.index(i) == 4:
            v5 = json.load(f)
        elif users.index(i) == 5:
            v6 = json.load(f)
